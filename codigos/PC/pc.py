...
@measure_performance
def experiment_pc(data, type_data="categoricos"):
    if type_data == "categoricos":
        pc = PC(data)
        model_pc_cat = pc.estimate(ci_test=CITests.chi_square,variant='orig',return_type='cpdag')
        print("Las aristas aprendidas por PC son:")
        print(model_pc_cat.edges())
        print(f"Ha detectado {len(model_pc_cat.edges)}")
    else:
        pc = PC(data)
        model_pc_nume = pc.estimate(variant='orig',ci_test=CITests.pearsonr,return_type='cpdag')
        print("Las aristas aprendidas por PC son:")
        print(model_pc_nume.edges())
        print(f"Ha detectado {len(model_pc_nume.edges)}")
